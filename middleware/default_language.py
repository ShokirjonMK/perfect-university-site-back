...
class CustomLocaleMiddleware(object):
    ...
    def __call__(self, request):
        """
        Redirect /en/*** url into uz. There-fore site will be available without en anytime using __call__ function
        """
        lang = str(request.path)
        lang = lang.split("/")
        if lang[1] == "en":
            path = request.path
            path = path.replace("/en/", "/api/uz/", 1)

            return HttpResponseRedirect(path)
        if lang[1] == "uz":
            path = request.path
            path = path.replace("/uz/", "/api/uz/", 1)

            return HttpResponseRedirect(path)

        if lang[1] == "ru":
            path = request.path
            path = path.replace("/ru/", "/api/ru/", 1)
