...
class QuizzCreate(HasRoleMixin, CreateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        api_url = "api/panel/quizz/api/"
        host = self.request.build_absolute_uri("/")[:-1]
        api = "%s/%screate/" % (host, api_url)
        post = "%s/%screate/" % (host, api_url)
        delete = "%s/%sdelete/" % (host, api_url)
        context["api"] = api
        context["post"] = post
...
class QuizzCreateAPI(HasRoleMixin, APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # quizz_data['is_published'] = BoolCheck(quizz_data['is_published'])
        # quizz_data['main_page'] = BoolCheck(quizz_data['main_page'])
        ...
        try:
            ...
            for answer in answers:
                ...
                if child.exists():
                    ...
                    del answer["id"]
                    child = question.Question.objects.create(
                        title_uz=answer["title_uz"], title_ru=answer["title_ru"], title_en=answer["title_en"], quizz=obj
                    )
            return Response(_("Muvaffaqiyatli saqlandi"), status=201)
        except Exception:
            return Response(_("Xatolik yuz berdi"), status=200)
